...
const renderLocations = () => {
  ...
  geoEvents.forEach((l) => {
    ...
    if (!seen.has(key)) { seen.add(key); uniqueGeo.push(l); }
  });

  if (!uniqueIPs.length && !uniqueGeo.length) {
    container.innerHTML = '<p style="color:var(--muted);padding:2rem">Brak danych o lokalizacji</p>';
    return;
  }
...
const exportData = () => {
  const data = { sessions: sessions.map((s) => ({ id: s.id, ip: s.ip, start: s.start, end: s.end, ua: s.ua, deviceInfo: s.deviceInfo, pageViews: s.pageViews.length, clicks: s.clicks, maxScroll: s.maxScroll })), events: allEvents };
...
const clearData = async () => {
  ...
  try {
    ...
    allEvents = []; sessions = [];
    renderStats(); renderSessions(); renderDevices(); renderLocations(); renderTable();
